...
import org.apache.kafka.common.network.{SSLFactory, PlainTextChannelBuilder, SSLChannelBuilder, ChannelBuilder, NetworkSend, Selector, NetworkReceive}
...
class RequestSendThread(val controllerId: Int,
  ...
  private def connectToBroker() {
    try {
      ...
      SelectorUtils.pollUntil(selector, config.controllerSocketTimeoutMs)(selector.connected().contains(connectionId))(time)
